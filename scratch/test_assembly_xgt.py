...
def test_assembly_xgt():
    ...
    body = b"\x00\x00\x00\x00\x58\x00\x02\x00\x00\x00\x01\x00\x05\x00%MW1\x02\x00\xC2\x01"
    req = header + body
    ...
    for i in range(10):
        ...
        body = b"\x00\x00\x00\x00\x54\x00\x02\x00\x00\x00\x01\x00\x05\x00%MW0"
        req = header + body
        s.sendall(req)
        resp = s.recv(1024)
        if len(resp) >= 30:
            var_len = resp[32] | (resp[33] << 8)
            data_len_offset = 34 + var_len
            comp = resp[data_len_offset+2] | (resp[data_len_offset+3] << 8)
